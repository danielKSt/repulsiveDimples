...
#'   nPoints <- 0
#'
#'   complete_owin <- owin(c(0,l), c(0,l))
#'   for (t_ind in 1:(length(snapshots)-1)) {
#'     complete_owin <- union.owin(complete_owin, owin(xrange = c(spacing*t_ind*l,(spacing*t_ind+1)*l), yrange = c(spacing*t_ind*l,(spacing*t_ind+1)*l)))
#'   }
#'
...
#'   combined.points <- ppp(x = combined.points$x, y = combined.points$y, window = complete_owin)
#'
#'   if(is.null(bw)){
#'     bw <- bw.pcf(combined.points, rmax = rMax)
#'   }
#'   return(Kest(combined.points, r = seq(from = 0, to = rMax, by = dr)))
#' }
#'
#'
#' #' Function for estimating the K-function given a list of point patterns
...
#'     bw <- bw.pcf(combined.points, rmax = rMax)
#'   }
#'   return(pcf(combined.points, r = seq(from = 0, to = rMax, by = dr), bw = bw, divisor = divisor))
#' }
